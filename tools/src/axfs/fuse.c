...
int axfs_rename(const char *oldpath, const char *newpath, unsigned int f){
        //resolve its path
        ...
        if(inum>>32==1){ //directory
                ...
                int count=0;
                for(int i = 0; i<4096; i++){
                        if(*(oldpath+i)==0){*(dotdot+i)='/';    count=i+1; break;}
                        *(dotdot+i) = *(oldpath+i);
                }
                *(dotdot+count)='.';
                *(dotdot+count+1)='.';
                ...
                //link .. to parent
                char* old = calloc(4096,1);
                strcpy(old, newpath);
                uint16_t last_slash_pos = 0;
                for (int i = 0; i<4096&&*old; i++){
                        if(*(old+i)=='/')
                                last_slash_pos = i;
                }
                *(old+last_slash_pos)=0;
                ...
                count=0;
                for(int i = 0; i<4096; i++){
                        if(*(newpath+i)==0){*(dotdot+i)='/';    count=i+1;      break;}
                        *(dotdot+i) = *(newpath+i);
                }
                *(dotdot+count)='.';
                *(dotdot+count+1)='.';
                printf("linking %s to %s\n", dotdot, old);
                axfs_link(old, dotdot);
                free(old);
                free(dotdot);
        }
        //unlink the old path
